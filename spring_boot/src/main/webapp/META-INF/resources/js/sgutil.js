...
/* 流量大小转换为M G格式 */
function as_countFLowSize(size) {
	var fsize = size;
	var fileSizeString;
	if (fsize % 1024 != 0 ) {
		fileSizeString = fsize + "M";
	} else if (fsize % 1024 == 0) {
		fileSizeString = (fsize / 1024) + "G";
	}else {
		fileSizeString = "0M";
	}
	return fileSizeString;
...
/*某个日期加上多少毫秒*/
 ...
 /*某个日期加上多少个月,这里是按照一个月31天来计算天数的*/
 _plusMonths : function(date,months){
	var dateTime = this._getTime(date);
	var mintimes = months*31*60*60*1000*24;
