...
public class HotDelPreTwo_DC_ClThread implements Runnable{
	...
	public void run() {
		...
		//初始化倍投阶梯
		String[] btStrArr = "4,14,50,177,628,2226,7892".split(",");
		...
		HotDelPreTwo_DC_ClThread.initTXFFCL();
	...
	public static void insertDownToTable(){
		...
		if(clMap!=null){
			HotClFrame.tableDefaultmodel.insertRow(0, new String[]{"--","--","--","--","--","--","--","--","--"});
			HotClFrame.tableDefaultmodel.insertRow(0, new String[]{nextFFCRound,clMap.get("position")+clMap.get("cl"),btArr[btNum].toString(),"--","--","--","待开奖","待开奖",moOrSzArr[mnOrSzFlag]});
		}
	}
